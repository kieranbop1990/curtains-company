...
export default function PendingCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setSurvey(initialValues.survey);
    setDrawingsApproved(initialValues.drawingsApproved);
    setDrawings(initialValues.drawings);
    setCompleted(initialValues.completed);
    setDepositPaid(initialValues.depositPaid);
    setOrderPlaced(initialValues.orderPlaced);
    setShippingDetails(initialValues.shippingDetails);
    setInvoices(initialValues.invoices);
    setQuotationDetails(initialValues.quotationDetails);
    setErrors({});
  };
  const validations = {
    purchaseOrder: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(new Pending(modelFields));
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "PendingCreateForm")}
      {...rest}
    >
      <SwitchField
